...
class Read_r {
    ...
    bool done, modified;
public:
    explicit Read_r(const lref<T>& obj, std::istream& inputStream=std::cin) : val(obj), in(inputStream), done(false), modified(false)
    { }
...
Read_r<std::string> read(const char* val) {
    return Read_r<std::string>(std::string(val),std::cin);
}
...
Read_r<std::string> readFrom(std::istream& inputStream, const char* val) {
    return Read_r<std::string>(std::string(val),inputStream);
}
