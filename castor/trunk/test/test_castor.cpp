...
void test_relational_loop() {
    ...
    relation r = child(ch,"mother1", f) /*&& write(ch, " ")*/ && predicate(&push, lst, ch) && False();
    if( r() )
...
void test_unique_f() {
    ...
    if(*i!=0 || *j!=3)
        throw "fa*iled test_un*ique_f 1";
    if(!r())
        throw "fa*iled test_un*ique_f 1";
    if(*i!=1 || *j!=3)
        throw "fa*iled test_un*ique_f 1";
...
void test_write_read() {
    { // write value to stream and the read it
	stringstream sstrm;
